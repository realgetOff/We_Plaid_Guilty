...
const GuessPrompt = ({ drawing, onDone }) =>
{
  ...
  useEffect(() =>
  {
    ...
    {
      if (guess)
        onDoneRef.current(guess);
      if (!guess)
        onDoneRef.current('(no guess)');
      return;
    }
    const id = setTimeout(() =>
    {
  ...
  let inputClass = 'guessprompt__input';
  if (error)
    inputClass = inputClass + ' guessprompt__input--error';

  let errorNode = null;
  if (error)
    errorNode = (
      <p className="guessprompt__error">⚠ {error}</p>
    );
  ...
  return (
    <div className="guessprompt">
      ...
      <div className="guessprompt__timer-track">
        <div
          className="guessprompt__timer-fill"
          style={{
            width: `${pct}%`,
        ...
        <div className="guessprompt__drawing-area">
          {drawing && (
            <img
              ...
              className="guessprompt__img"
            />
          )}
          {!drawing && (
            <span className="guessprompt__placeholder">
              No drawing received.
            </span>
          )}
        </div>
